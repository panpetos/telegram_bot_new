...
async def save_tg_file_to_uploads(file_id: str, context: ContextTypes.DEFAULT_TYPE) -> tuple[str, str]:
    ...
    tg_url = f.file_path if f.file_path.startswith("http") else f"https://upload.wikimedia.org/wikipedia/commons/8/83/Telegram_2019_Logo.svg"
...
# ───────── детекция окон ─────────
def detect_windows_opencv(image_path: str) -> List[Dict[str, Any]]:
    """Детекция окон с помощью OpenCV"""
    try:
        ...
        for contour in contours:
            ...
            if len(approx) >= 4:
                ...
                if w > width * 0.03 and h > height * 0.03 and w < width * 0.7 and h < height * 0.7:
        ...
        log.error(f"Ошибка детекции окон: {e}")
        return []
...
async def got_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Детектируем окна
        windows = detect_windows_opencv(local_path)
